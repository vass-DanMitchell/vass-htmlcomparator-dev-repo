...
namespace HtmlDiff
{
    public class HtmlDiff
    {
        ...
        public string Build()
        {
            ...
            string aux = _content.ToString();

            string delStype = "<span tag-del style='color: #ff0000;text-decoration: line-through;font-weight: bold'>";
            string insStyle = "<span tag-ins style='color: #ff0000;text-decoration: underline red;'>";
            string delStypeSapn = "<span tag-del style='color: #ff0000;text-decoration: line-through;font-weight: bold'><label>image deleted<label><img";


            aux = aux.Replace("<del class='diffmod'><img", delStypeSapn);
            aux = aux.Replace("</del>", "</span>");

            aux = aux.Replace("<ins class='diffins'>", insStyle);
            aux = aux.Replace("</ins>", "</span>");

            aux = aux.Replace("<del class='diffmod'>", delStype);
            aux = aux.Replace("</del>", "</span>"); 

            aux = aux.Replace("<del class='diffdel'>", delStype);
            aux = aux.Replace("</del>", "</span>");

            aux = aux.Replace("<ins class='diffmod'>", insStyle);
            aux = aux.Replace("</ins>", "</span>");
